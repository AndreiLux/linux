	...
	if(IPU_TO_START!= ioread32((void *)((unsigned long)adapter->config_reg_virt_addr + IPU_START_REG))) {
		...
		if (kfifo_peek(&FIFO_TaskElements, &head)) {
			...
			if (0 == ret) {
				...
				ipu_watchdog_start(&adapter->resume_wtd, WATCHDOG_TIMEOUT_THD_MS);
			} else {
...
static irqreturn_t ipu_interrupt_handler(int irq, void *dev)
{
	...
	ipu_watchdog_stop(&adapter->resume_wtd);
...
static void ipu_resume(unsigned long para)
{
...
static ssize_t ipu_write_when_ipu_up(struct cambricon_ipu_private *dev, const char __user *buf, size_t count, loff_t *f_pos)
{
	unsigned long bytes_not_copied;
	unsigned max_buff_size = dev->inst_ram_size;
	unsigned long corrected_pos;
	unsigned long corrected_diff;
	...
	/* ipu on, write data to inst ram directly */
	if (down_interruptible(&dev->inst_ram_sem)) {
		return -ERESTARTSYS;
	}

	if (((unsigned long)*f_pos % 4) != 0) {
		/* |......................|......................|................|    */
		/* ^corrected_pos(4x)     ^f_pos                 ^4(x+1)          ^end */
		/* |<-- corrected_diff -->|<- 4-corrected_diff ->|<-count-(4-CD)->|    */
		/*     (short CD)         |<------------- count ----------------->|    */

		/* for all positive numbers x and y, x >= (x%y) is always true, so "-" can not cause OVERFLOW ERROR */
		corrected_diff = (unsigned long)*f_pos % 4;
		corrected_pos = (unsigned long)*f_pos - corrected_diff;

		/* read reg: the value will be partially-coveraged by user data [4x to f_pos] */
		temp_reg = ioread32((void *)((unsigned long)dev->inst_ram_virt_addr + corrected_pos));

		/* set reg: length: 4-corrected_diff, here (4-corrected_diff)>= 0 is always true [f_pos to 4(x+1)] */
		temp_buf = (char *)&temp_reg;
		bytes_not_copied = copy_from_user((void *)&temp_buf[corrected_diff], buf, 4 - corrected_diff);
		if (bytes_not_copied) {
			printk(KERN_ERR"[%s]: IPU_ERROR:Copy data from user to temp_buf failed, bytes_not_copied=0x%lx\n", __func__, bytes_not_copied);
			up(&dev->inst_ram_sem);
			return 0; /* return 0 means fail and copied 0 bytes to register */
		}
		iowrite32(temp_reg, (void *)((unsigned long)dev->inst_ram_virt_addr + corrected_pos));

		/* set reg: [4(x+1) to end] */
		bytes_not_copied = copy_from_user((void *)((unsigned long)dev->inst_ram_virt_addr + (corrected_pos + 4)),
			&buf[4 - corrected_diff], count - (4 - corrected_diff));

		printk(KERN_ERR"[%s]: IPU_WARN: alignment correction, *f_pos=0x%pK, count=0x%x, corrected_pos=0x%pK\n",
			__func__, (void *)*f_pos, (unsigned int)count, (void *)corrected_pos);
	} else {
		bytes_not_copied = copy_from_user((void *)((unsigned long)dev->inst_ram_virt_addr + (unsigned long)(*f_pos)), buf, count);
	}
	up(&dev->inst_ram_sem);
	...
	if (bytes_not_copied) {
		...
		/* here type convert no risk of type truncation, because the caller "ipu_write" had judged the value */
		return (ssize_t)count - (ssize_t)bytes_not_copied;
	} else {
...
static ssize_t ipu_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (adapter->ipu_power_up) {
		ret_value = ipu_write_when_ipu_up(dev, buf, count, f_pos);
		mutex_unlock(&adapter->power_mutex);
	} else {
...
static int cambricon_ipu_probe(struct platform_device *pdev)
{
	...
	ipu_watchdog_init(&adapter->resume_wtd, adapter->feature_tree.soft_watchdog_enable, ipu_resume);
