...
static struct dss_comm_mmbuf_info g_pre_online_mmbuf[DSS_CHN_MAX_DEFINE] = {{0}};
	...
	char __iomem *dfc_base, dss_dfc_t *s_dfc)
{
	...
	if ((g_dss_version_tag == FB_ACCEL_KIRIN970) && (hisifd->index != EXTERNAL_PANEL_IDX)){
	  ...
	  dss_overlay_block_t *pov_h_block, dss_rect_t *wb_ov_block_rect, int ovl_idx, int ov_h_block_idx)
{
	...
	ovl_bg_color = (g_dss_version_tag & FB_ACCEL_KIRIN970) ?
	...
	dss_wb_layer_t *layer, dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	if (layer->need_cap & CAP_HFBCE) {
		int ret = hisi_dss_hfbce_config(hisifd, pov_req, layer, aligned_rect, ov_block_rect, last_block);
		if (ret != 0) {
			HISI_FB_ERR("hisi_dss_hfbcd_config failed! ret = %d\n", ret);
			return -EINVAL;
		}
	...
	dss_overlay_t *pov_req, uint32_t cmdlist_pre_idxs, bool enable_cmdlist, int *use_comm_mmbuf)
{
			...
			if ((ovl_idx == DSS_OVL0) || (ovl_idx == DSS_OVL1)) {
				if ((layer->need_cap & (CAP_AFBCD | CAP_HFBCD)) && (layer->dst_rect.y >= pov_h_block->ov_block_rect.y)) {
					if (j < DSS_CHN_MAX_DEFINE) {
						g_pre_online_mmbuf[j].mmbuf.addr = layer->img.mmbuf_base;
						g_pre_online_mmbuf[j].mmbuf.size = layer->img.mmbuf_size;
						g_pre_online_mmbuf[j].ov_idx = ovl_idx;
						j++;
					}
				}
			}
	...
	if (g_debug_dump_mmbuf) {
		if ((ovl_idx == DSS_OVL0) || (ovl_idx == DSS_OVL1)) {
			for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
				HISI_FB_INFO("pre_mmbuf[%d].addr=0x%x, size=%d, ovl_idx = %d, j=%d, frame_no=%d\n",
					i, g_pre_online_mmbuf[i].mmbuf.addr, g_pre_online_mmbuf[i].mmbuf.size, ovl_idx, j, pov_req->frame_no);
			}
		}
	}
	...
	if (pov_req->wb_enable && ((ovl_idx > DSS_OVL1) || (!has_ovl))) {
		...
		for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
			for (k = 0; k < DSS_CHN_MAX_DEFINE; k++) {
				if ((((offline_mmbuf[i].addr < g_pre_online_mmbuf[k].mmbuf.addr + g_pre_online_mmbuf[k].mmbuf.size) &&
					(offline_mmbuf[i].addr >= g_pre_online_mmbuf[k].mmbuf.addr))
					|| ((g_pre_online_mmbuf[k].mmbuf.addr < offline_mmbuf[i].addr + offline_mmbuf[i].size) &&
					(g_pre_online_mmbuf[k].mmbuf.addr >= offline_mmbuf[i].addr)))
					&& offline_mmbuf[i].size) {
					if (use_comm_mmbuf) {
						*use_comm_mmbuf = 1 << g_pre_online_mmbuf[k].ov_idx;
						break;
					}
				}
			}
		...
		if (g_debug_dump_mmbuf) {
			for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
				HISI_FB_INFO("offline_mmbuf[%d].addr=0x%x, size=%d, j=%d, frame_no=%d\n",
					i, offline_mmbuf[i].addr, offline_mmbuf[i].size, j, pov_req->frame_no);
			}
		}
